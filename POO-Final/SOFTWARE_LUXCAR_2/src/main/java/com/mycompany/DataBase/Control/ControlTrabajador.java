...
public class ControlTrabajador {
    ...
    public void InsertarTrabajador(TrabajadorClass Trabajador) {
        PreparedStatement pst = null;
        try {
            String sql = "INSERT INTO Trabajador (DNI, Nombres, Apellidos, FechaNacimiento, Direccion, Correo, Telefono, FechaContratacion, Puesto, HoraCobro, Genero, Estado)"
                    + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            pst = Data.getConection().prepareStatement(sql);
    ...
    public void ModificarTrabajador(TrabajadorClass Trabajador) {
        ...
        try {
            String sql = "UPDATE Trabajador "
                    + "SET Nombres = ?, Apellidos = ?, FechaNacimiento = ?, Direccion = ?, Correo = ?, Telefono = ?, FechaContratacion = ?, Puesto = ?, HoraCobro = ?, Genero = ?, Estado = ? "
                    + "WHERE DNI = ?";
            pst = Data.getConection().prepareStatement(sql);
            ...
            pst.setString(12, Trabajador.getDNI());
