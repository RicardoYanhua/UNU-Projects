...
public class SpinnerAño extends JSpinner {

    public SpinnerAño() {
        setOpaque(false);
        int AñoActual = LocalDate.now().getYear(); 
        setModel(new SpinnerNumberModel(1900, 1900, AñoActual + 20, 1)); 
        setValue(AñoActual); 
        setUI(new SpinnerUIAño());
    }

    public void setTextEditor(String text) {
        SpinnerUIAño.Editor editor = (SpinnerUIAño.Editor) getEditor();
        editor.setText(text);
    }
...
class SpinnerUIAño extends BasicSpinnerUI {
        ...
        private void UpdateValueSpinner() {
            try {
                ...
                if (String.valueOf(value).length() == 4) {
                    if (value <= LocalDate.now().getYear()+20 && value > 1900) {
                        spinner.setValue(value);
                    } else if (value > LocalDate.now().getYear()+20) {
                        spinner.setValue(LocalDate.now().getYear()+20);
                        txtrepaint();
                    } else if (value < 1900) {
                        spinner.setValue(1900);
                        txtrepaint();
                    }else {
                        txtrepaint();
                    }
    ...
    private class CellTextUI extends JTextField {
        ...
        public CellTextUI() {
            ...
            setDocument(new TxtIntegerSize(4) {
            });
