...
public class SpinnerDia extends JSpinner {

    public SpinnerDia() {
        setOpaque(false);
        int DiaActual = LocalDate.now().getDayOfMonth(); // Obtiene el dia actual
        setModel(new SpinnerNumberModel(1, 1, 31, 1)); // Rango de 1 a 31
        setValue(DiaActual); // Valor inicial
        setUI(new SpinnerUIDia());
    }

    public void setTextEditor(String text) {
        SpinnerUIDia.Editor editor = (SpinnerUIDia.Editor) getEditor();
        editor.setText(text);
    }
...
class SpinnerUIDia extends BasicSpinnerUI {
