...
function FormatoSoloLetrasMayusculas(event) {
	let input = event.target.value;
	input = input.replace(/[^a-zA-ZÁÉÍÓÚÑáéíóúñ\s]/g, '');
...
function AplicarFormtatos() {
	...
	const TextoMayusculasInputs = document.querySelectorAll('.FormatoTextoMayusculas');
	TextoMayusculasInputs.forEach(input => {
		input.addEventListener('input', FormatoSoloLetrasMayusculas);
	});
}
