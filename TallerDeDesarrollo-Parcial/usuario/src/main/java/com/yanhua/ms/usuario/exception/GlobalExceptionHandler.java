package com.yanhua.ms.usuario.exception;
...
public class GlobalExceptionHandler {
    ...
    protected ResponseEntity<ErrorResponse> handleIllegalArgument(IllegalArgumentException ex,
        ...
        /**
         * Maneja `UnauthorizedException` lanzada por la aplicación -> 401 Unauthorized.
         *
         * Cuándo se ejecuta:
         * - Cuando el usuario no está autorizado para acceder a un recurso (fallos en autenticación/autorización).
         *
         * Qué registra y devuelve:
         * - Registra un `WARN` con el mensaje de la excepción.
         * - Devuelve un `ErrorResponse` con `status = 401` y el mensaje de la excepción.
         */
    @ExceptionHandler(UnauthorizedException.class)
    protected ResponseEntity<ErrorResponse> handleUnauthorized(UnauthorizedException ex,
            HttpServletRequest request) {
        String path = request.getRequestURI();
        logger.warn("[GlobalExceptionHandler][handleUnauthorized][WARN] → Acceso no autorizado en {}: {}", path, ex.getMessage());
        ErrorResponse body = ErrorResponse.builder()
                .timestamp(LocalDateTime.now())
