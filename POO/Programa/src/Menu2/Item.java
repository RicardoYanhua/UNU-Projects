...
public class Item extends JButton {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (!mainMenu) {
            ...
            g2.setColor(new Color(245, 249, 250));
            int height = getHeight();
            int size = 8;
            int y = (height - size) / 2;
            g2.drawOval(50, y, size, size);
            g2.setColor(mainColor);
            if (isSelected()) {
                alpha = 1;
            }
            g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, alpha));
            g2.fillOval(50, y, size + 1, size + 1);
            g2.dispose();
        } else {
            if (isSelected()) {
                Graphics2D g2 = (Graphics2D) g.create();
    ...
    public void setGoogleIcon(GoogleMaterialDesignIcon icon) {
        if (icon != null) {
            this.icon = icon;
            setIcon(new GoogleMaterialIcon(icon, GradientType.HORIZONTAL, SystemColor.MAIN_COLOR_3, SystemColor.MAIN_COLOR_4, 22).toIcon());
        }
    }
